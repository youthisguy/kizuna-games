...
fn fen(env: &Env) -> soroban_sdk::String {
    soroban_sdk::String::from_str(env, "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w - - 0 1")
}
...
fn test_commit_moves_alternates_turns() {
    ...
    // White opens
    client.commit_move(&id, &white, &san(&env, "e4"), &fen(&env));
    assert_eq!(client.get_move_count(&id), 1u32);
    assert_eq!(client.get_turn(&id), black);

    // Black responds
    client.commit_move(&id, &black, &san(&env, "e5"), &fen(&env));
    assert_eq!(client.get_move_count(&id), 2u32);
    assert_eq!(client.get_turn(&id), white);

    // Scholar's mate setup
    client.commit_move(&id, &white, &san(&env, "Qh5"), &fen(&env));
    client.commit_move(&id, &black, &san(&env, "Nc6"), &fen(&env));
    client.commit_move(&id, &white, &san(&env, "Bc4"), &fen(&env));
    client.commit_move(&id, &black, &san(&env, "Nf6"), &fen(&env));
...
fn test_stranger_cannot_move() {
    ...
    let id = client.create_game(&white, &black, &1u64, &0u64);
    client.commit_move(&id, &stranger, &san(&env, "e4"), &fen(&env));
}
...
fn test_get_moves_returns_full_history() {
    ...
    let id = client.create_game(&white, &black, &1u64, &0u64);
    client.commit_move(&id, &white, &san(&env, "d4"), &fen(&env));
    client.commit_move(&id, &black, &san(&env, "d5"), &fen(&env));
    client.commit_move(&id, &white, &san(&env, "c4"), &fen(&env));
