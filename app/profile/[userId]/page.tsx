...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "stats" | "games" | "achievements"
  >("stats");
  const ACHIEVEMENT_META: Record<string, { label: string; icon: any; color: string; glow: string }> = {
    "FirstWin": { 
      label: "Vanguard", 
      icon: Trophy, 
  ...
  const isOwnProfile =
    connectedAddress && (userId === connectedAddress || userId === user?.id);
      ...
      <div className="relative max-w-3xl mx-auto px-4 py-8 pb-32">
        ...
        >
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            ...
            <div
              className={`flex sm:flex-col items-center sm:items-end justify-between sm:justify-start px-4 py-2.5 sm:py-3 rounded-2xl border ${
                netPositive
            ...
            >
              <p
                className="text-[9px] uppercase tracking-widest sm:mb-1"
                style={{ color: netPositive ? "#6ee7b7" : "#fca5a5" }}
              >
                Net P&L
              </p>
          ...
          <div className="mt-5 space-y-1">
            ...
            <div className="h-1.5 rounded-full bg-zinc-800 overflow-hidden">
              <motion.div
                initial={{ width: 0 }}
                animate={{
                  width: `${Math.min((user.elo_rating / 2000) * 100, 100)}%`,
                }}
        ...
        >
          {[
            ...
            <div
              key={label}
              className="border border-zinc-800/50 rounded-2xl p-3 text-center bg-zinc-900/20"
            >
              ...
              <div className="border border-zinc-800/50 rounded-2xl p-5 bg-zinc-900/20 space-y-4">
                <div className="grid grid-cols-2 gap-4 text-[10px]">
                  {[
                    {
                      label: "Total Staked",
                      value: `${stroopsToXlm(user.total_staked ?? 0)} XLM`,
                      color: "text-zinc-200",
                    },
                    {
                      label: "Total Won",
                      value: `${stroopsToXlm(user.total_won ?? 0)} XLM`,
                      color: "text-emerald-400",
                    },
                    {
                      label: "Total Lost",
                      value: `${stroopsToXlm(user.total_lost ?? 0)} XLM`,
                      color: "text-rose-400",
                    },
                    {
                      label: "Net Profit",
                      value: `${netPositive ? "+" : "-"}${netXlm} XLM`,
                      color: netPositive ? "text-emerald-400" : "text-rose-400",
                    },
                  ].map(({ label, value, color }) => (
                    <div key={label}>
                ...
                <div className="grid grid-cols-2 gap-4 text-[10px]">
                  {[
                    ...
                    <div key={label}>
                      ...
                      <p className="font-black text-base text-zinc-200">
                        {value}
                      </p>
                    </div>
                  ))}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="h-2 rounded-full overflow-hidden flex">
                      ...
                      <div
                        ...
                        style={{
                          width: `${
                            ((user.total_draws ?? 0) / user.total_games) * 100
                          }%`,
                        }}
                      />
                      <div
                        ...
                        style={{
                          width: `${
                            (user.total_losses / user.total_games) * 100
                          }%`,
                        }}
                      />
                    </div>
              ...
              {recentGames.length === 0 ? (
                ...
                recentGames.map((g: any) => {
                  ...
                  return (
                    <div
                      key={g.id}
                      className={`border rounded-2xl p-4 ${resultBg} transition-colors`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <div
                            className={`text-xs font-black uppercase px-2 py-1 rounded-lg border ${resultBg} ${resultColor} shrink-0`}
                          >
                            {result}
                          </div>
                          <div className="min-w-0">
                            <p className="text-[11px] text-zinc-300 font-mono truncate">
                              vs {formatAddress(opp ?? "Unknown")}
                            </p>
                        ...
                        <div className="shrink-0 text-right">
                          ...
                          >
                            {won
                              ? `+${stroopsToXlm(g.winner_payout ?? 0)}`
                              : draw
                              ? "±0"
                              : `-${stroopsToXlm(g.stake_each ?? 0)}`}
                          </p>
                          <p className="text-[9px] text-zinc-600">XLM</p>
                        </div>
                      </div>
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-zinc-800/40">
                        <p className="text-[9px] text-zinc-700">
                          {timeAgo(g.finished_at)}
                        </p>
                        <button
                          onClick={() =>
                            router.push(`/play/${g.escrow_game_id}`)
                          }
  ...
  >
    {achievements.length === 0 ? (
      <div className="border border-dashed border-zinc-800 rounded-3xl p-12 text-center bg-zinc-900/20">
        <div className="relative inline-block">
          <Star size={32} className="text-zinc-800 mb-3" />
          <motion.div 
        ...
        <p className="text-zinc-500 text-sm font-medium">No achievements unlocked</p>
        <p className="text-[10px] text-zinc-700 mt-1 uppercase tracking-widest">
          Conquer the board to earn your rank
        </p>
      </div>
    ) : (
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {achievements.map((a: any, i: number) => {
          const meta = ACHIEVEMENT_META[a.achievement_type] ?? ACHIEVEMENT_META.default;
          const IconTag = meta.icon;
          ...
          return (
            <motion.div
              ...
              whileHover={{ y: -5 }}
              className={`relative group overflow-hidden border rounded-2xl p-5 flex flex-col items-center justify-center transition-all duration-300 ${meta.color} hover:border-white/20`}
            >
              {/* Background Ambient Glow */}
              <div 
                className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                style={{ background: `radial-gradient(circle at center, ${meta.glow} 0%, transparent 70%)` }}
              />
              ...
              <div className="relative mb-3">
                ...
                <motion.div
                  ...
                  className="absolute inset-[-8px] border border-dashed border-current opacity-20 rounded-full group-hover:opacity-40"
                />
                ...
                >
                  <IconTag size={28} strokeWidth={1.5} className="drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]" />
                </motion.div>
              </div>

              {/* Text Info */}
              <p className="text-[10px] font-black uppercase tracking-[0.15em] text-white/90 relative z-10">
                {meta.label}
              </p>
              
              <div className="flex items-center gap-1 mt-1.5 opacity-50 group-hover:opacity-80 transition-opacity">
                 <div className="w-1 h-1 rounded-full bg-current" />
                 <p className="text-[8px] font-mono whitespace-nowrap">
                   {timeAgo(a.achieved_at)}
                 </p>
              </div>
